...
export class ReportsService {
  ...
  private async gather(user: AuthenticatedUser) {
    ...
    const [university, faculty, overview, settings] = await Promise.all([
      ...
      this.settings.getMap(universityId),
    ]);
    return { university, faculty, overview, settings };
  }
  ...
  async attendancePdf(user: AuthenticatedUser, byId?: string, byName?: string, lang: ReportLang = 'th'): Promise<{ buffer: Buffer; reportNumber: string }> {
    const universityId = user.universityId;
    const { university, faculty, overview, settings } = await this.gather(user);
    ...
    doc.font(F).fontSize(15).fillColor('#4a5666')
      .text(
        settings['pdf.header']
          ? `${L(lang, 'รายงานสรุปการเข้าเรียน', 'Attendance Summary Report')} · ${settings['pdf.header']}`
          : L(lang, 'รายงานสรุปการเข้าเรียน', 'Attendance Summary Report'),
        100, 92, { width: pageW - 240, align: 'center', lineBreak: false, ellipsis: true },
      );
    ...
    doc.font(F).fontSize(9).fillColor('#a0aab8')
      .text(`${settings['pdf.footer']} · ${reportNumber} · verify at ${verifyUrl}`, left, doc.page.height - 58, { width: right - left, align: 'center', lineBreak: false });
  ...
  private async gatherStudent(universityId: string, studentId: string) {
    const [university, faculty, student, settings] = await Promise.all([
    ...
    return { university, faculty, student, settings, total, present, late, absent, rate, tier: tierOf(rate), bySubject, log };
  }
  ...
  async studentPdf(universityId: string, studentId: string, byId?: string, byName?: string, lang: ReportLang = 'th'): Promise<{ buffer: Buffer; reportNumber: string }> {
    ...
    doc.font(F).fontSize(14).fillColor('#4a5666').text(
      d.settings['pdf.header']
        ? `${L(lang, 'รายงานการเข้าเรียนรายบุคคล', 'Individual Attendance Report')} · ${d.settings['pdf.header']}`
        : L(lang, 'รายงานการเข้าเรียนรายบุคคล', 'Individual Attendance Report'),
      100, 86, { width: pageW - 230, align: 'center', lineBreak: false, ellipsis: true },
    );
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.font(F).fontSize(9).fillColor('#a0aab8')
        .text(`${d.settings['pdf.footer']} · ${reportNumber} · verify at ${verifyUrl} · ${L(lang, 'หน้า', 'Page')} ${i + 1}/${range.count}`, left, doc.page.height - 40, { width: right - left, align: 'center', lineBreak: false });
    }
    doc.flushPages();
  ...
  private async gatherSectionGrades(user: AuthenticatedUser, sectionId: string) {
    const universityId = user.universityId;
    const [university, faculty, section, settings] = await Promise.all([
      this.prisma.university.findUnique({ where: { id: universityId }, select: { nameEn: true, nameTh: true } }),
      this.prisma.faculty.findFirst({ where: { universityId, code: 'NURSING' }, select: { nameEn: true, nameTh: true } }),
    ...
    return { university, faculty, section, settings, rubrics, students };
  }
  ...
  async sectionGradesPdf(user: AuthenticatedUser, sectionId: string, byId?: string, byName?: string, lang: ReportLang = 'th'): Promise<{ buffer: Buffer; reportNumber: string }> {
    ...
    doc.font(F).fontSize(12).fillColor('#4a5666')
      .text(
        `${L(lang, 'รายงานผลการเรียนราย Section', 'Section Grade Report')} — ${d.section.subject.code} ${(lang === 'en' ? d.section.subject.nameEn : d.section.subject.nameTh) ?? d.section.subject.nameEn} · Sec ${d.section.sectionNo}`
          + (d.settings['pdf.header'] ? ` · ${d.settings['pdf.header']}` : ''),
        88, 71, { width: pageW - 236, align: 'center', lineBreak: false, ellipsis: true },
      );
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.page.margins.bottom = 0;
      doc.font(F).fontSize(9).fillColor('#a0aab8')
        .text(`${d.settings['pdf.footer']} · ${reportNumber} · verify at ${verifyUrl} · ${L(lang, 'หน้า', 'Page')} ${i + 1}/${range.count}`, left, doc.page.height - 30, { width: right - left, align: 'center', lineBreak: false });
    }
    doc.flushPages();
  ...
  async studentGradePdf(user: AuthenticatedUser, studentId: string, sectionId: string, byId?: string, byName?: string, lang: ReportLang = 'th'): Promise<{ buffer: Buffer; reportNumber: string }> {
    const universityId = user.universityId;
    const [university, faculty, section, summary, settings] = await Promise.all([
      this.prisma.university.findUnique({ where: { id: universityId }, select: { nameEn: true, nameTh: true } }),
      this.prisma.faculty.findFirst({ where: { universityId, code: 'NURSING' }, select: { nameEn: true, nameTh: true } }),
    ...
    doc.font(F).fontSize(14).fillColor('#4a5666').text(
      settings['pdf.header']
        ? `${L(lang, 'รายงานผลการเรียนรายบุคคล', 'Individual Grade Report')} · ${settings['pdf.header']}`
        : L(lang, 'รายงานผลการเรียนรายบุคคล', 'Individual Grade Report'),
      100, 86, { width: pageW - 230, align: 'center', lineBreak: false, ellipsis: true },
    );
    doc.moveTo(left, 112).lineTo(right, 112).strokeColor('#ff8a4c').lineWidth(2).stroke();
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.page.margins.bottom = 0;
      doc.font(F).fontSize(9).fillColor('#a0aab8')
        .text(`${settings['pdf.footer']} · ${reportNumber} · verify at ${verifyUrl} · หน้า ${i + 1}/${range.count}`, left, doc.page.height - 40, { width: right - left, align: 'center', lineBreak: false });
    }
    doc.flushPages();

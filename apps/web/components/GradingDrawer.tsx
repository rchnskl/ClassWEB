...
interface Item { id: string; textEn: string; textTh: string | null; weightPercent: number; maxRating: number }
...
interface Summary {
  ...
  rubrics: { rubricId: string; nameEn: string; nameTh: string | null; weightPercent: number; scorePercent: number | null; graded: boolean; contribution: number }[];
}
                  ...
                  <div key={s.id} style={{ marginBottom: 14 }}>
                    ...
                    {s.items.map((it) => (
                      <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderBottom: '1px solid var(--glass-hairline)' }}>
                        <div style={{ flex: 1, fontSize: 12.5, lineHeight: 1.35 }}>{name(it.textEn, it.textTh)}</div>

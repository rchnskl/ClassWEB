    ...
    <div style={{ display: 'flex', gap: 16, padding: 16, maxWidth: 1440, margin: '0 auto' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="rise" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18, gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ fontSize: 27, fontWeight: 750, letterSpacing: -0.6, margin: 0 }}>Students</h1>
            <p className="muted" style={{ margin: '4px 0 0', fontSize: 14.5 }}>
              {total} student{total === 1 ? '' : 's'} · Faculty of Nursing
            </p>
          </div>
        ...
        {showForm && (
          <form onSubmit={submitForm} className="glass rise" style={{ padding: 20, marginBottom: 18, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, alignItems: 'end' }}>
            <Field label="Student ID *"><input className="input" required value={form.studentCode} onChange={(e) => setForm({ ...form, studentCode: e.target.value })} placeholder="6510100" /></Field>
            <Field label="English name *"><input className="input" required value={form.nameEn} onChange={(e) => setForm({ ...form, nameEn: e.target.value })} placeholder="Full name" /></Field>
            <Field label="Nickname"><input className="input" value={form.nickname} onChange={(e) => setForm({ ...form, nickname: e.target.value })} placeholder="—" /></Field>
            <Field label="Program *">
            ...
            <button className="btn-primary" type="submit" disabled={saving} style={{ padding: '12px', fontSize: 14.5 }}>{saving ? 'Saving…' : 'Create'}</button>
        ...
        <div className="glass rise" style={{ padding: 8, overflow: 'hidden' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ textAlign: 'left', color: 'var(--text-2)' }}>
                  <Th>Student ID</Th><Th>Name</Th><Th>Nickname</Th><Th>Program</Th><Th>Year</Th><Th>Status</Th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center' }} className="muted">Loading…</td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={6} style={{ padding: 48, textAlign: 'center' }}>
                    <div className="brand-gradient" style={{ width: 46, height: 46, borderRadius: 14, margin: '0 auto 12px', display: 'grid', placeItems: 'center' }}><IconStudents width={22} height={22} /></div>
                    <div style={{ fontWeight: 650 }}>No students found</div>
                    <div className="muted" style={{ fontSize: 13, marginTop: 4 }}>Try a different search, or add a student.</div>
                  </td></tr>
                ) : rows.map((s) => (
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <PagerBtn disabled={skip === 0 || loading} onClick={() => load(Math.max(0, skip - PAGE), search)}>Previous</PagerBtn>
              <PagerBtn disabled={skip + PAGE >= total || loading} onClick={() => load(skip + PAGE, search)}>Next</PagerBtn>
            </div>
          </div>
        </div>
      </div>
